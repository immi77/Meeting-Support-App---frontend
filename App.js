...
export default function App() {
    ...
    const [urlHandle, setUrlHandle] = useState(null)
    ...
    //             Navigation.navigate("Join", queryParams);
    //         }
    //     }
    // }).catch(console.error);
    ...
    useEffect(() => {
        ...
        function handleDeepLink(event) {
            ...
            if(HttpClient?.meetingId?.toLowerCase() == data?.queryParams?.meetingId?.toLocaleLowerCase()) {
                ...
                Navigation.navigate("Main", data?.queryParams);
                return;
            }
            ...
            if (data?.queryParams?.meetingId && HttpClient.meetingId) {
                Navigation.navigate("Index", { message: "Do you want to leave the team and join another meeting?", functionToCall: HttpClient.leaveMeeting, params: ["JoinScreen", data?.queryParams] });
            }
